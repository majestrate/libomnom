...
files="omnom tests pybind"

cd "$(dirname $0)/../"
if [ "$1" = "verify" ] ; then
    exit $($binary --output-replacements-xml $(find $files | grep -E '\.[hc](pp)?$' | grep -v '\#') | grep '</replacement>' | wc -l)
else
    $binary -i $(find $files | grep -E '\.[hc](pp)?$' | grep -v '\#') &> /dev/null
fi
