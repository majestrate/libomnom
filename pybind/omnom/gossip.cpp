        ...
        py::class_<Gossiper>(mod, "Gossiper")
            .def(
                ...
                [](Gossiper& self, Entity ent, std::function<bool(const PeerInfo&)> filter) {
                    ...
                    self.Broadcast(std::move(ent), [filter](auto, const PeerState& state) {
                        try
                        {
            ...
            .def("add_entity_handler", [](Gossiper& self, EntityKind kind, py::function func) {
                auto handler = [func = std::move(func)](const PeerState& state, Entity ent) {
                    try
                    {
                        py::gil_scoped_acquire acquire;
