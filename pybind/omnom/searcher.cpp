        ...
        py::class_<EntitySearcher>(mod, "Searcher")
            .def(
                ...
                [](EntitySearcher& self, EntityKind kind, EntityID id) {
                    ...
                    self.MaybeObtainEntityByID(std::move(kind), std::move(id), [&promise](std::optional<Entity> result) { promise.set_value(result); });
                    return promise.get_future().get();
                })
            .def("find_highest_id", [](EntitySearcher& self, EntityKind kind) -> EntityID {
