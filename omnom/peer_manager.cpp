...
namespace omnom
{
    ...
    PeerInfo::to_bt_value() const
    {
        ...
        oxenmq::bt_dict value{{"uid", std::string{reinterpret_cast<const char*>(uid.data()), uid.size()}}, {"addrs", bt_addrs}};
        return value;
    }
    ...
    {
        oxenmq::bt_dict value{{"addr", addr}, {"rank", rank}};
        return value;
    }
    ...
    PeerManager::PeerManager(Context* ctx) : _ctx{ctx}
    {}
    ...
    PeerManager::Start()
    {
        ...
        _ctx->AddRequestHandler("register_conn", [this](oxenmq::Message& msg) { HandleRegisterConn(msg); });

        _ctx->AddRequestHandler("list_peers", [this](oxenmq::Message& msg) { HandleListPeers(msg); });

        _ctx->lmq().add_timer([this]() { Tick(); }, 100ms, true, *m_Logic);
    }
    ...
    PeerManager::HandleRegisterConn(oxenmq::Message& msg)
    {
        ...
        CallSafe([this, info = std::move(info), conn = msg.conn, &reply]() {
            try
            {
    ...
    PeerManager::OnNewOutboundPeer(oxenmq::ConnectionID id, oxenmq::address addr)
    {
        ...
        _ctx->Request(
            id,
            "register_conn",
            [this, id = id, addr = addr.zmq_address()](bool success, std::vector<std::string> data) {
                ...
                CallSafe([this, success, info, id, addr, KillConnection]() {
                    try
                    {
                        if (not success)
    ...
    PeerManager::Tick()
    {
        for (const auto& item : m_OutboundPeerAttempts)
        {
            ...
            _ctx->lmq().connect_remote(
                addr,
                [this, addr, persist](oxenmq::ConnectionID conn) {
                    LogInfo(_ctx, "connected to ", addr, " via ", conn);
                    CallSafe([this, conn, addr, persist]() {
                        OnNewOutboundPeer(conn, addr);
                        m_Peers[conn].persist = persist;
                    });
                },
                [this, addr](oxenmq::ConnectionID, std::string_view reason) {
                    LogInfo(_ctx, "did not connect to ", addr, ": ", reason);
                    CallSafe([this, addr]() { m_Limiter.MarkConnectFail(addr.zmq_address()); });
                });
        }
    }
